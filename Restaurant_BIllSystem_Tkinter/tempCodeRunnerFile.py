...
def billarea(self):
    ...
    self.txtarea.insert(END, line_format.format("Product", "Qty", "Price"))
    self.txtarea.insert(END, "="*38 + "\n")

    items = []

    # Starter
    if self.nutella.get() != 0:
        items.append(("Samosa", self.nutella.get(), self.nu))
    if self.noodles.get() != 0:
        items.append(("Paneer Tikka", self.noodles.get(), self.no))
    if self.lays.get() != 0:
        items.append(("Chicken Tikka", self.lays.get(), self.la))
    if self.oreo.get() != 0:
        items.append(("Veg Pakora", self.oreo.get(), self.ore))
    if self.muffin.get() != 0:
        items.append(("Papdi Chaat", self.muffin.get(), self.mu))
    if self.silk.get() != 0:
        items.append(("Tomato Soup", self.silk.get(), self.si))
    if self.namkeen.get() != 0:
        items.append(("Masala Papad", self.namkeen.get(), self.na))

    # Main Course
    if self.atta.get() != 0:
        items.append(("Butter Chicken", self.atta.get(), self.at))
    if self.pasta.get() != 0:
        items.append(("Pasta", self.pasta.get(), self.pa))
    if self.rice.get() != 0:
        items.append(("Basmati Rice", self.rice.get(), self.ri))
    if self.oil.get() != 0:
        items.append(("Paneer Masala", self.oil.get(), self.oi))
    if self.sugar.get() != 0:
        items.append(("Palak Paneer", self.sugar.get(), self.su))
    if self.dal.get() != 0:
        items.append(("Daal", self.dal.get(), self.da))
    if self.tea.get() != 0:
        items.append(("Chole Bhature", self.tea.get(), self.te))

    # Snacks
    if self.soap.get() != 0:
        items.append(("Noodles", self.soap.get(), self.so))
    if self.shampoo.get() != 0:
        items.append(("Aloo Tikki Chaat", self.shampoo.get(), self.sh))
    if self.lotion.get() != 0:
        items.append(("Dahi Vada", self.lotion.get(), self.lo))
    if self.cream.get() != 0:
        items.append(("Pav Bhaji", self.cream.get(), self.cr))
    if self.foam.get() != 0:
        items.append(("Bhel Puri", self.foam.get(), self.fo))
    if self.mask.get() != 0:
        items.append(("Soup", self.mask.get(), self.ma))
    if self.sanitizer.get() != 0:
        items.append(("Pokara", self.sanitizer.get(), self.sa))

    # Insert items line by line with same alignment
    for name, qty, price in items:
        self.txtarea.insert(END, line_format.format(name, qty, price))

    self.txtarea.insert(END, "-"*38 + "\n")
    if self.a.get() != "0.0 Rs":
        self.txtarea.insert(END, f"Snacks Tax           : {self.a.get()}\n")
    if self.b.get() != "0.0 Rs":
        self.txtarea.insert(END, f"Grocery Tax          : {self.b.get()}\n")
    if self.c.get() != "0.0 Rs":
        self.txtarea.insert(END, f"Beauty & Hyg. Tax    : {self.c.get()}\n")
